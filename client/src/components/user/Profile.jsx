...
export default function Profile() {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col md:flex-row items-center md:items-start justify-between gap-6">
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left w-full">
          ...
          <div className="relative group w-24 h-24 shrink-0 rounded-2xl overflow-hidden border border-slate-200 shadow-sm bg-slate-50">
            <img
              src={avatarUrl}
              alt="Avatar"
          ...
          <div className="flex-1 min-w-0 space-y-2">
            ...
            <div className="pt-2 flex flex-wrap items-center justify-center sm:justify-start gap-x-2 gap-y-1 text-xs">
              ...
              {nextRank && (
                <div className="flex items-center gap-2 sm:ml-2">
                  <span className="text-[10px] text-slate-400 font-bold uppercase">{rank.name}</span>
                  <div className="h-1.5 w-24 sm:w-28 rounded-full bg-slate-100 overflow-hidden inline-block align-middle">
                    <div
                      className="h-full rounded-full transition-all duration-500"
                      style={{ width: `${progressPct}%`, backgroundColor: rank.color }}
                    />
                  </div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase">{nextRank.name}</span>
                  <span className="text-[10px] text-slate-400">({nextRank.minRep - reputation} điểm nữa)</span>
                </div>
              )}
            </div>
          </div>
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded-xl border px-4 py-2 text-xs font-bold shadow-sm transition-all shrink-0 ${
            activeTab === 'edit'
      ...
      {activeTab === 'profile' && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-3 text-center">
                <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                  <div className="text-lg font-extrabold text-slate-800">{reputation}</div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
              <h3 className="text-sm font-bold text-slate-800 border-b border-slate-100 pb-2 mb-3 flex items-center gap-1.5">
                <span className="material-symbols-outlined text-lg text-slate-500">military_tech</span>
                Cấp bậc hiện tại
              </h3>
              <div className="flex items-center gap-4">
                <div className="shrink-0 bg-slate-50 border border-slate-100 rounded-2xl p-2.5 flex items-center justify-center">
                  <ReputationBadge reputation={reputation} size="lg" showLabel={false} />
                </div>
                <div>
                  <div className="font-bold text-slate-800 text-sm">Cấp bậc: {rank.name}</div>
