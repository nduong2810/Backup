    ...
    return (
        <AppCard title="Đăng ký tài khoản" subtitle="Tham gia diễn đàn sinh viên IT">
            ...
            <form className="mt-4 space-y-4" onSubmit={(e) => { e.preventDefault(); onSubmit(); }}>
                ...
                <InputField label="Mật khẩu" name="password" type="password" value={form.password} onChange={(e) => onFieldChange('password', e.target.value)} allowPasswordToggle required disabled={loading} />
                <InputField label="Xác nhận mật khẩu" name="confirmPassword" type="password" value={form.confirmPassword} onChange={(e) => onFieldChange('confirmPassword', e.target.value)} allowPasswordToggle required disabled={loading} />
