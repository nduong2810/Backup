...
const reportReasons = [
  ...
  { value: 'duplicate', label: 'Trùng câu hỏi/bài viết đã có' },
  { value: 'very_low_quality', label: 'Chất lượng rất thấp, khó cứu vãn' },
  { value: 'moderator_attention', label: 'Cần moderator xem thủ công' },
];

export default function ReportPostModal({ isOpen, onClose, onSubmitReport }) {
  useEffect(() => {
    if (isOpen) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-[460px] bg-white border border-slate-200 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-modal-enter text-slate-800">
        ...
        <div className="relative flex items-center justify-center px-6 py-4 border-b border-slate-100">
          <h2 className="text-md font-bold text-slate-900 text-center">Báo cáo bài viết</h2>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4 max-h-[75vh]">
          <div className="space-y-1.5">
            <h3 className="text-base font-bold text-slate-900 leading-snug">Tại sao bạn báo cáo bài viết này?</h3>
            <p className="text-[12.5px] text-slate-500 leading-relaxed font-body-sm">
              Báo cáo này giúp ban quản trị nhận biết và xử lý bài viết vi phạm tiêu chuẩn cộng đồng hoặc nội dung không lành mạnh trên diễn đàn.
            </p>
          ...
          <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden bg-slate-50/20">
            {reportReasons.map((reason) => (
              <button
                ...
                className="w-full flex items-center justify-between text-left px-4 py-3 hover:bg-slate-50 active:bg-slate-100 transition-all group"
              >
