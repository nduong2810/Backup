...
const formatStateValue = (key, value) => {
  ...
  if (key === 'isActive') return value ? 'Hoạt động' : 'Bị khóa';
  if (key === 'statusReason' || key === 'reason') return value;
  ...
  return (
    <div className="flex max-w-full flex-wrap gap-1 overflow-hidden">
      {entries.map(([key, val]) => (
        <span
          key={key}
          className="inline-flex max-w-full min-w-0 items-start gap-1 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-bold leading-5 text-slate-600"
        >
          <span className="shrink-0 text-slate-400">{formatKeyLabel(key)}:</span>
...
export default function AdminAuditLogsTab({ embedded = false }) {
  ...
  const handleMouseDown = (colKey, event) => {
    ...
    const handleMouseMove = (moveEvent) => {
      ...
      const newWidth = Math.max(110, startWidth + deltaX);
  ...
  const handleDoubleClick = (colKey) => {
    ...
    cells.forEach((cell) => {
      const contentEl = cell.querySelector('.audit-cell-content');
      const contentWidth = contentEl ? contentEl.scrollWidth + 42 : cell.scrollWidth;
      if (contentWidth > maxWidth) maxWidth = contentWidth;
    });

    const finalWidth = Math.min(520, Math.max(110, maxWidth));
  ...
  return (
    <section
      className={
        embedded
          ? 'flex flex-col gap-6'
          : 'mx-auto w-full max-w-[1400px] px-6 py-8 flex flex-col gap-6'
      }
    >
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <h1 className="mt-1 text-2xl font-extrabold text-slate-900 leading-none">
                Nhật ký thao tác quản trị
              </h1>
              <p className="mt-1.5 text-sm text-slate-500">
                Theo dõi admin nào khóa user, đổi status bài, duyệt/từ chối donation và lý do xử lý.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="space-y-4">
          ...
          <form
            ...
            className="grid w-full grid-cols-1 gap-3 xl:grid-cols-[minmax(240px,1fr),190px,170px,160px,160px,104px]"
          >
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="overflow-x-auto scrollbar-custom pb-2">
          <table
            className="table-fixed w-full divide-y divide-slate-100"
            style={{ minWidth: `${totalWidth}px` }}
          >
            <thead className="bg-slate-50/80">
              <tr className="text-left text-xs font-bold uppercase tracking-wider text-slate-500">
                <th
                  className="relative overflow-hidden border-r border-slate-200/50 px-5 py-4 select-none last:border-r-0"
                  style={{ width: `${colWidths.time}px` }}
                  data-col="time"
                >
                  <div className="audit-cell-content truncate">Thời gian</div>
                  <div
                    onMouseDown={(e) => handleMouseDown('time', e)}
                    onDoubleClick={() => handleDoubleClick('time')}
                    className="absolute right-0 top-0 bottom-0 z-10 w-1.5 cursor-col-resize transition-colors hover:bg-primary/50 active:bg-primary"
                ...
                <th
                  className="relative overflow-hidden border-r border-slate-200/50 px-5 py-4 select-none last:border-r-0"
                  style={{ width: `${colWidths.admin}px` }}
                  data-col="admin"
                >
                  <div className="audit-cell-content truncate">Admin</div>
                  <div
                ...
                <th
                  className="relative overflow-hidden border-r border-slate-200/50 px-5 py-4 select-none last:border-r-0"
                  style={{ width: `${colWidths.action}px` }}
                  data-col="action"
                >
                  <div className="audit-cell-content truncate">Hành động</div>
                  <div
                ...
                <th
                  ...
                  data-col="target"
                >
                  <div className="audit-cell-content truncate">Đối tượng</div>
                  <div
                ...
                <th
                  className="relative overflow-hidden border-r border-slate-200/50 px-5 py-4 select-none last:border-r-0"
                  style={{ width: `${colWidths.reason}px` }}
                  data-col="reason"
                >
                  <div className="audit-cell-content truncate">Lý do</div>
                  <div
                ...
                <th
                  className="relative overflow-hidden border-r border-slate-200/50 px-5 py-4 select-none last:border-r-0"
                  style={{ width: `${colWidths.change}px` }}
                  data-col="change"
                >
                  <div className="audit-cell-content truncate">Thay đổi</div>
                  <div
              ...
              {loading &&
                Array.from({ length: 5 }).map((_, index) => (
                  <tr key={index} className="animate-pulse">
                    <td className="px-5 py-5" data-col="time">
                      <div className="h-4 w-28 rounded bg-slate-100" />
                    </td>
                    <td className="px-5 py-5" data-col="admin">
                      <div className="h-4 w-36 rounded bg-slate-100" />
                    </td>
                    <td className="px-5 py-5" data-col="action">
                      <div className="h-7 w-28 rounded-full bg-slate-100" />
                    </td>
                    <td className="px-5 py-5" data-col="target">
                      <div className="h-4 w-32 rounded bg-slate-100" />
                    </td>
                    <td className="px-5 py-5" data-col="reason">
                      <div className="h-4 w-40 rounded bg-slate-100" />
                    </td>
                    <td className="px-5 py-5" data-col="change">
                ...
                <tr>
                  <td colSpan={6} className="px-5 py-12 text-center">
                    ...
                    <p className="mt-1 text-xs text-slate-400">
                      Thử đổi bộ lọc hoặc thực hiện một thao tác quản trị mới.
                    </p>
                  </td>
                </tr>
              )}
              ...
              {!loading &&
                logs.map((log) => (
                  <tr key={log._id} className="transition hover:bg-slate-50">
                    <td
                      className="overflow-hidden px-5 py-5 align-top text-xs font-semibold text-slate-500"
                      data-col="time"
                    >
                      <div className="audit-cell-content truncate whitespace-nowrap">
                        {formatDateTime(log.createdAt)}
                      </div>
                    </td>

                    <td
                      className="min-w-0 overflow-hidden px-5 py-5 align-top"
                      data-col="admin"
                    >
                      <div className="audit-cell-content max-w-full min-w-0 overflow-hidden">
                        <p
                          className="truncate text-sm font-bold text-slate-800"
                          title={log.actor?.fullName || log.actor?.email || 'Không rõ admin'}
                        >
                          {log.actor?.fullName || 'Không rõ admin'}
                        </p>
                        <p
                    ...
                    <td
                      className="overflow-hidden px-5 py-5 align-top"
                      data-col="action"
                    >
                      <div className="audit-cell-content max-w-full overflow-hidden">
                        <span
                    ...
                    >
                      <div className="audit-cell-content max-w-full min-w-0 overflow-hidden">
                        ...
                        <p
                          className="mt-1 line-clamp-2 text-sm font-semibold leading-5 text-slate-700 break-words"
                    ...
                    <td
                      ...
                      data-col="reason"
                    >
                      <div className="audit-cell-content max-w-full min-w-0 overflow-hidden font-normal">
                        <p
                          className="line-clamp-3 text-sm leading-5 text-slate-700 break-words"
                          title={log.reason || 'Không có lý do'}
                        >
                      ...
                      <div className="audit-cell-content max-w-full min-w-0 overflow-hidden">
                        <div className="space-y-1.5 text-xs">
                          <div className="flex max-w-full items-start gap-1 overflow-hidden">
                            <span className="w-12 shrink-0 font-bold text-slate-700">
                              Trước:
                            </span>
                            <div className="min-w-0 flex-1 overflow-hidden">
                              {renderStateInfo(log.previousState)}
                            </div>
                          </div>
