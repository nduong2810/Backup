      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {post.tags.slice(0, 5).map((tag) => (
                <Link
                  ...
                  className="rounded-md bg-secondary-fixed px-2 py-1 font-label-mono text-label-mono text-[#39739d] hover:bg-secondary-fixed/80"
                >
                  #{tag}
                </Link>
              ))}
        ...
        <div className="grid grid-cols-3 gap-2 text-center sm:w-56">
          <div className="rounded-lg bg-slate-50 px-2 py-2">
            <div className="text-sm font-bold text-slate-900">{formatNumber(reactionCount)}</div>
            <div className="text-[11px] text-slate-500">{reactionLabel}</div>
          </div>
          <div className="rounded-lg bg-slate-50 px-2 py-2">
            <div className="text-sm font-bold text-slate-900">{formatNumber(post.answerCount)}</div>
            <div className="text-[11px] text-slate-500">{commentLabel}</div>
          </div>
          <div className="rounded-lg bg-slate-50 px-2 py-2">
            <div className="text-sm font-bold text-slate-900">{formatNumber(post.viewCount)}</div>
...
export default function AuthorProfilePage() {
  ...
  const roleBadgeClass = isAdminProfile
  ...
  return (
    <div className="mx-auto max-w-6xl pt-2 pb-8">
      <div className="mb-5 flex items-center justify-between gap-3">
        ...
        <Link
          to="/user/profile"
          className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:border-slate-300 hover:bg-slate-50"
        >
          Hồ sơ của tôi
        </Link>
      </div>
      ...
      {isAdminProfile ? (
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-950 via-slate-900 to-amber-950 p-6 text-white shadow-[0_18px_50px_-32px_rgba(15,23,42,0.6)] sm:p-8">
          <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center">
            <img
              src={getAvatarUrl(user, 180, '1d4ed8')}
              alt={user.fullName}
              className="h-28 w-28 rounded-2xl border border-white/15 object-cover shadow-lg"
            />
            <div className="min-w-0">
              <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-extrabold ring-1 ${roleBadgeClass}`}>
                <span className="material-symbols-outlined text-[15px]">verified_user</span>
                {roleLabel}
              </span>
              <h1 className="mt-3 break-words text-3xl font-black leading-tight text-white sm:text-4xl">{user.fullName}</h1>
              <p className="mt-2 text-sm font-medium text-white/75">{user.email}</p>
              {user.bio && <p className="mt-4 whitespace-pre-wrap text-sm leading-7 text-white/70">{user.bio}</p>}
            </div>
          </div>
        </section>
      ) : (
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_18px_50px_-32px_rgba(15,23,42,0.55)]">
          <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-blue-900 px-6 py-8 text-white sm:px-8">
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div className="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center">
                <img
                  src={getAvatarUrl(user, 180, '0066cc')}
                  alt={user.fullName}
                  className="h-28 w-28 rounded-2xl border-4 border-white/25 object-cover shadow-lg"
                />
                <div className="min-w-0">
              ...
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 md:w-[460px]">
                ...
                <div className="rounded-xl bg-white/12 p-3 text-center ring-1 ring-white/15">
                  <div className="text-2xl font-black">{formatNumber(postSummary.totalUpvotes)}</div>
                  <div className="text-xs text-white/70">upvote</div>
                </div>
                <div className="rounded-xl bg-white/12 p-3 text-center ring-1 ring-white/15">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                {formatNumber(postsPagination.total)} bài viết đang hiển thị trên diễn đàn
              </p>
            </div>
            {totalPages > 1 && (
          ...
          {posts.length === 0 ? (
            ...
            <div className="space-y-3">
              {posts.map((post) => <AuthorPostCard key={post._id} post={post} />)}
            </div>
          )}
          ...
          {totalPages > 1 && (
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-slate-100 pt-4">
              <button
                ...
                disabled={!canGoPrev}
                className="rounded-lg border border-slate-200 px-3 py-2 text-sm font-bold text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50"
              >
                Trước
              </button>
                ...
                {paginationItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`min-w-9 rounded-lg border px-3 py-2 text-sm font-bold ${isActive ? 'border-primary bg-primary text-white' : 'border-slate-200 text-slate-600 hover:bg-slate-50'}`}
                    >
                      {item}
                    </button>
                  );
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
              ...
              {donations.length === 0 ? (
                <div className="mt-5 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm text-slate-500">
                  Chưa có lượt ủng hộ nào được duyệt cho người dùng này.
                </div>
              ) : (
                  ...
                  {donations.slice(0, 5).map((donation) => (
                    <div key={donation._id} className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="truncate text-sm font-bold text-slate-900">
                            {donation.donor?.fullName || donation.donorSnapshot?.fullName || 'Ẩn danh'}
                          </div>
                          <div className="mt-1 text-xs text-slate-500">{new Date(donation.createdAt).toLocaleString('vi-VN')}</div>
                        </div>
                      ...
                      <p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-500">
                        {donation.post?.title || donation.postSnapshot?.title || 'Bài viết được ủng hộ'}
                      </p>
                    </div>
                  ))}
                </div>
